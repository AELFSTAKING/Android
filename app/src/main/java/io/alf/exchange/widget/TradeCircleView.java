...
public class TradeCircleView extends View {
     ...
     * @param context
     */
    public TradeCircleView(Context context) {
        this(context, null);
    }
     ...
     * @param attrs   自定义属性
     */
    public TradeCircleView(Context context, AttributeSet attrs) {
     ...
     */
    public TradeCircleView(Context context, AttributeSet attrs, int defStyleAttr) {
        super(context, attrs, defStyleAttr);
        TypedArray ta = context.getTheme().obtainStyledAttributes(attrs, R.styleable.circleProgressBar,
                defStyleAttr, 0);
        if (ta != null) {
            firstColor = ta.getColor(R.styleable.circleProgressBar_firstColor, context.getResources().getColor(R.color.textThird)); // 默认底色为亮灰色
            secondColor = ta.getColor(R.styleable.circleProgressBar_secondColor, context.getResources().getColor(R.color.color_decrease)); // 默认进度条颜色为蓝色
            circleWidth = ta.getDimensionPixelSize(R.styleable.circleProgressBar_circleWidth, (int) TypedValue.applyDimension(
                    TypedValue.COMPLEX_UNIT_DIP, 3, getResources().getDisplayMetrics())); // 默认圆弧宽度为6dp
            ta.recycle();
        }
    ...
    private void drawCircle(Canvas canvas, int center, int radius) {
        ...
        int color;
        if (isFlag) {
            color = getResources().getColor(R.color.color_increase);
            colorArray = new int[]{color, color};//
        } else {
            color = getResources().getColor(R.color.color_decrease);
            colorArray = new int[]{color, color};//
        }
        ...
        circlePaint.setColor(color); // 设置圆弧的颜色
    ...
    private void drawText(Canvas canvas, int center, int radius) {
        ...
        String percent;
        if (!TextUtils.isEmpty(tvprogress)) {
            float v = Float.parseFloat(tvprogress);
            percent = String.format("%.1f", v) + "%";//%.1f
        } else {
            percent = String.format("%.1f", result) + "%";//%.1f
        }
...
//        textPaint.setColor(secondColor); // 设置文字颜色
        if (isFlag) {
            textPaint.setColor(getResources().getColor(R.color.color_increase));
        } else {
            textPaint.setColor(getResources().getColor(R.color.color_decrease));
        }
        textPaint.setTextSize(DensityUtil.dp2px(9)); // 设置要绘制的文字大小
     ...
     * @param color
     */
    public void setSecondColor(int color, boolean isFlag) {
        this.isFlag = isFlag;
    ...
    public void setProgress(String progress) {
        ...
        if (!TextUtils.isEmpty(progress)) {
            tvprogress = BigDecimalUtil.mulTrade(progress, String.valueOf(100), 2);
            percent = (int) Double.parseDouble(tvprogress);
        }
        if (percent < 0) {
            percent = 0;
        }
     ...
     */
    public void setProgress(String progress, boolean useAnimation) {
        int percent = 0;
        if (!TextUtils.isEmpty(progress)) {
            String s = BigDecimalUtil.mulTrade(progress, String.valueOf(100), 1);
            percent = (int) Double.parseDouble(s);
        }
        if (percent < 0) {
            percent = 0;
        }
