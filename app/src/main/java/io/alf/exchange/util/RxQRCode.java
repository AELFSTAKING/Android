...
public class RxQRCode {
    ...
    public static Bitmap createQRCode(CharSequence content, int QR_WIDTH, int QR_HEIGHT,
            int backgroundColor, int codeColor) {
        Bitmap bitmap = null;
        try {
            // 判断URL合法性
     ...
     * @param ivCode    图片空间
     */
    public static void createQRCode(String content, int QR_WIDTH, int QR_HEIGHT, ImageView ivCode) {
        ivCode.setImageBitmap(createQRCode(content, QR_WIDTH, QR_HEIGHT));
    }
    ...
    public static class Builder {
        ...
        public Bitmap into(ImageView imageView) {
            Bitmap bitmap = RxQRCode.createQRCode(content, codeSide, codeSide, backgroundColor,
                    codeColor);
            if (imageView != null) {
