...
public class Keccak {
    ...
    private static final long[] RC =
            {0x0000000000000001L, 0x0000000000008082L, 0x800000000000808AL, 0x8000000080008000L,
                    ...
                    0x8000000000000080L, 0x000000000000800AL, 0x800000008000000AL,
                    0x8000000080008081L,
    ...
    private int rateSizeBitsFor(int digestSizeBits) {
        ...
        switch (digestSizeBits) {
            ...
            case 512:
                return 576;
            default:
                throw new IllegalArgumentException("Invalid digestSizeBits: " + digestSizeBits
                        + " ⊄ { 128, 224, 256, 288, 384, 512 }");
        }
        //@formatter:on
    }
    ...
    private void reset(int rateSizebits, int digestSizeBits) {
        if (rateSizebits + digestSizeBits * 2 != MAX_STATE_SIZE) {
            throw new IllegalArgumentException(
                    "Invalid rateSizebits + digestSizeBits * 2: " + rateSizebits + " + "
    ...
    private void update(ByteBuffer in) {
        ...
        if ((rateBits & 0x7)
                > 0) //this could be implemented but would introduce considerable performance
    ...
    private ByteBuffer digest(int outSize, boolean direct) {
        ByteBuffer buffer = direct ? ByteBuffer.allocateDirect(outSize) : ByteBuffer.allocate(
                outSize);
        digest(buffer);
        buffer.flip();
        return buffer;
        ...
        if (!padded) {
            ...
            if ((rateBits & 0x7) > 0) {
                throw new IllegalStateException(
                        "Cannot digest while in bit-mode"); //this could be implemented but would
            }
            // introduce considerable performance degradation - also, it's never technically
            // possible.
