...
public class Crypto<T extends KDFParams> {
    ...
    private static Crypto createCrypto(String password, byte[] origin, String kdfType,
            boolean isCached) {
        Crypto crypto = PBKDF2Crypto.PBKDF2.equals(kdfType) ? PBKDF2Crypto.createPBKDF2Crypto()
                : SCryptCrypto.createSCryptCrypto();
