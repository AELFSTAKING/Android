...
public class Identity {
    ...
    public static Identity createIdentity(String name, String password, String passwordHit,
            String network, String segWit, boolean createEthWallet) {
        ...
        Identity identity = new Identity(metadata, mnemonicCodes, password, createEthWallet);
    ...
    public static Identity recoverIdentity(String mnemonic, String name, String password,
        ...
        metadata.setNetwork(network);
        metadata.setSegWit(segWit);
        Identity identity = new Identity(metadata, mnemonicCodes, password, false);
        currentIdentity = identity;
        return identity;
    }
    ...
    public void setAccountName(String name) {
        this.keystore.getMetadata().setName(name);
        flush();
    }
        ...
        for (String chainType : chainTypes) {
            ...
            switch (chainType) {
                ...
                case ChainType.TRON:
                    ...
                    break;
                case ChainType.ZIL:
                    wallet = deriveZILLWallet(mnemonics, password);
                    break;
                case ChainType.HPB:
                    wallet = deriveHpbWallet(mnemonics, password);
                    break;
                default:
                    throw new TokenException(
                            String.format("Doesn't support deriving %s wallet", chainType));
            }
            addWallet(wallet);
            wallets.add(wallet);
        }
    ...
    private Wallet deriveBitcoinWallet(List<String> mnemonicCodes, String password, String segWit) {
        ...
        IMTKeystore keystore = HDMnemonicKeystore.create(walletMetadata, password, mnemonicCodes,
                path);
    ...
    public Wallet deriveEthereumWallet(List<String> mnemonics, String password) {
        ...
        IMTKeystore keystore = V3MnemonicKeystore.create(walletMetadata, password, mnemonics,
    ...
    private Wallet deriveEOSWallet(List<String> mnemonics, String password) {
        ...
        IMTKeystore keystore = EOSKeystore.create(metadata, password, "", mnemonics,
                BIP44Util.EOS_LEDGER, null);
        return WalletManager.createWallet(keystore);
    }

    /**
     * Tick.Du add the code for Tron.
     */
    public Wallet deriveTRONWallet(List<String> mnemonics, String password) {
        Metadata metadata = new Metadata();
        metadata.setChainType(ChainType.TRON);
        metadata.setPasswordHint(this.getMetadata().getPasswordHint());
        metadata.setSource(this.getMetadata().getSource());
        metadata.setNetwork(this.getMetadata().getNetwork());
        metadata.setName("TRON");
        IMTKeystore keystore = V3MnemonicKeystore.create(metadata, password, mnemonics,
                BIP44Util.TRON_PATH);
        return WalletManager.createWallet(keystore);
    }

    /**
     * Tick.Du add the code for Zilliqa.
     */
    private Wallet deriveZILLWallet(List<String> mnemonics, String password) {
        Metadata metadata = new Metadata();
        metadata.setChainType(ChainType.ZIL);
        metadata.setPasswordHint(this.getMetadata().getPasswordHint());
        metadata.setSource(this.getMetadata().getSource());
        metadata.setNetwork(this.getMetadata().getNetwork());
        metadata.setName("ZILLIQA");
        IMTKeystore keystore = V3MnemonicKeystore.create(metadata, password, mnemonics,
                BIP44Util.ZILL_PATH);
        return WalletManager.createWallet(keystore);
    }
    ...
    /**
     * Tick.Du add the code for Hpb.
     */
    private Wallet deriveHpbWallet(List<String> mnemonics, String password) {
        Metadata metadata = new Metadata();
        metadata.setChainType(ChainType.HPB);
        metadata.setPasswordHint(this.getMetadata().getPasswordHint());
        metadata.setSource(this.getMetadata().getSource());
        metadata.setNetwork(this.getMetadata().getNetwork());
        metadata.setName("HPB");
        IMTKeystore keystore = V3MnemonicKeystore.create(metadata, password, mnemonics,
                BIP44Util.HPB_PATH);
        return WalletManager.createWallet(keystore);
    }
    ...
    String encryptDataToIPFS(String originData, long unixtime, byte[] iv) {
        ...
        byte[] ciphertext = AES.encryptByCBC(originData.getBytes(Charset.forName("UTF-8")), encKey,
                iv);
        ...
        int totalLen =
                (int) (headerLength + ciphertextLength.getSizeInBytes() + ciphertextLength.value
                        + 65);
    ...
    public String decryptDataFromIPFS(String encryptedData) {
        ...
        byte[] encKey = Arrays.copyOf(encryptionKey, 16);
        String content = new String(AES.decryptByCBC(ciphertext, encKey, iv),
                Charset.forName("UTF-8"));

        byte[] signature = new byte[65];
        System.arraycopy(payload, srcPos, signature, 0, 65);
        try {
            BigInteger pubKey = EthereumSign.ecRecover(NumericUtil.bytesToHex(toSign),
                    NumericUtil.bytesToHex(signature));
            ECKey ecKey = ECKey.fromPublicOnly(ByteUtil.concat(new byte[]{0x04},
                    NumericUtil.bigIntegerToBytesWithZeroPadded(pubKey, 64)));
            String recoverIpfsID = new Multihash(Multihash.Type.sha2_256,
    ...
    public String signAuthenticationMessage(int accessTime, String deviceToken, String password) {
        ...
        String data = String.format(Locale.ENGLISH, "%d.%s.%s", accessTime, getIdentifier(),
                deviceToken);
        return EthereumSign.sign(data, decrypted);
    }

    //Tick.Du: Add verifyPassword method.
    ...
    public boolean changePassword(String oldPassword, String newPassword) {
        if (verifyPassword(oldPassword)) {
            this.keystore = keystore.changePassword(oldPassword, newPassword);
            List<Wallet> walletList = getWallets();
            if (walletList != null && walletList.size() > 0) {
                for (Wallet wallet : walletList) {
                    WalletManager.changePassword(wallet.getId(), oldPassword, newPassword);
                }
            }
            flush();
            currentIdentity = null;
