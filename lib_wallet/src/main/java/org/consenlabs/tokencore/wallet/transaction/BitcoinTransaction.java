...
public class BitcoinTransaction implements TransactionSigner {
    ...
    public TxSignResult signTransaction(String chainID, String password, Wallet wallet) {
        ...
        for (int i = 0; i < getOutputs().size(); i++) {
            ...
            BigInteger privateKey = wallet.getMetadata().getSource().equals(Metadata.FROM_WIF)
                    ? prvKeys.get(0) : prvKeys.get(i);
            ECKey ecKey;
            if (output.getAddress().equals(ECKey.fromPrivate(privateKey).toAddress(
                    network).toBase58())) {
                ecKey = ECKey.fromPrivate(privateKey);
            } else if (output.getAddress().equals(ECKey.fromPrivate(privateKey, false).toAddress(
                    network).toBase58())) {
            ...
            Script scriptPubKey = ScriptBuilder.createOutputScript(
                    Address.fromBase58(network, output.getAddress()));
            Sha256Hash hash = tran.hashForSignature(i, scriptPubKey, Transaction.SigHash.ALL,
                    false);
            ECKey.ECDSASignature ecSig = ecKey.sign(hash);
            TransactionSignature txSig = new TransactionSignature(ecSig, Transaction.SigHash.ALL,
                    false);
    ...
    public TxSignResult signSegWitTransaction(String chainId, String password, Wallet wallet) {
        ...
        if (toAddress.isP2SHAddress()) {
            targetScriptPubKey = ScriptBuilder.createP2SHOutputScript(
                    toAddress.getHash160()).getProgram();
        } else {
            targetScriptPubKey = ScriptBuilder.createOutputScript(toAddress).getProgram();
        }

        byte[] changeScriptPubKey = ScriptBuilder.createP2SHOutputScript(
                changeAddress.getHash160()).getProgram();
...
//      stream.write(changeScriptPubKey);
            ...
            for (int i = 0; i < getOutputs().size(); i++) {
                ...
                BigInteger prvKey = Metadata.FROM_WIF.equals(wallet.getMetadata().getSource())
                        ? prvKeys.get(0) : prvKeys.get(i);
                ECKey key = ECKey.fromPrivate(prvKey, true);
                String redeemScript = String.format("0014%s",
                        NumericUtil.bytesToHex(key.getPubKeyHash()));
                ...
                // calc scriptCode
                byte[] scriptCode = NumericUtil.hexToBytes(String.format("0x1976a914%s88ac",
                        NumericUtil.bytesToHex(key.getPubKeyHash())));
            ...
            for (int idx = 0; idx < 2; idx++) {
                ...
                for (int i = 0; i < getOutputs().size(); i++) {
                    ...
                    // the length of byte array that follows, and this length is used by
                    // OP_PUSHDATA1
                    stream.write(0x17);
                    // the length of byte array that follows, and this length is used by cutting
                    // array
                    stream.write(0x16);
                ...
                if (idx == 0) {
                    for (int i = 0; i < witnesses.size(); i++) {
                        BigInteger prvKey = Metadata.FROM_WIF.equals(
                                wallet.getMetadata().getSource()) ? prvKeys.get(0) : prvKeys.get(i);
            ...
            String txHash = NumericUtil.bytesToHex(
                    Sha256Hash.hashTwice(serialStreams[1].toByteArray()));
    ...
    private void collectPrvKeysAndAddress(String segWit, String password, Wallet wallet) {
        this.network =
                wallet.getMetadata().isMainNet() ? MainNetParams.get() : TestNet3Params.get();
        if (wallet.getMetadata().getSource().equals(Metadata.FROM_WIF)) {
            changeAddress = Address.fromBase58(network, wallet.getAddress());
            BigInteger prvKey = DumpedPrivateKey.fromBase58(network,
                    wallet.exportPrivateKey(password)).getKey().getPrivKey();
            ...
            DeterministicKey indexKey = HDKeyDerivation.deriveChildKey(changeKey,
                    new ChildNumber(getChangeIdx(), false));
            ...
            for (UTXO output : getOutputs()) {
                ...
                DeterministicKey accountKey = HDKeyDerivation.deriveChildKey(xprvKey,
                        new ChildNumber(accountIdx, false));
                DeterministicKey externalChangeKey = HDKeyDerivation.deriveChildKey(accountKey,
                        new ChildNumber(changeIdx, false));
