...
public class V3MnemonicKeystore extends IMTKeystore implements EncMnemonicKeystore,
        ExportableKeystore {
    ...
    private V3MnemonicKeystore(Metadata metadata, String password, List<String> mnemonicCodes,
        ...
        this.address = AddressCreatorManager.getInstance(metadata.getChainType(),
                metadata.isMainNet(), metadata.getSegWit()).fromPrivateKey(prvKeyBytes);
    ...
    public static V3MnemonicKeystore create(Metadata metadata, String password,
            List<String> mnemonicCodes, String path) {
    ...
    public Keystore changePassword(String oldPassword, String newPassword) {
        String mnemonic = new String(
                getCrypto().decryptEncPair(oldPassword, this.getEncMnemonic()));
