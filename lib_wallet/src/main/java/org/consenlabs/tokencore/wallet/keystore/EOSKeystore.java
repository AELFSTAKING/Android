...
public class EOSKeystore extends IMTKeystore implements EncMnemonicKeystore {
    ...
    public static final String PERM_ACTIVE = "active";
    ...
    private EOSKeystore(Metadata metadata, String password, String accountName,
            List<String> prvKeys, List<PermissionObject> permissions, String id) {

        this.crypto = Crypto.createPBKDF2CryptoWithKDFCached(password,
                NumericUtil.generateRandomBytes(128));
    ...
    private EOSKeystore(Metadata metadata, String password, String accountName,
            List<String> mnemonics, String path, List<PermissionObject> permissions, String id) {
    ...
    public static EOSKeystore create(Metadata metadata, String password, String accountName,
            List<String> mnemonics, String path, List<PermissionObject> permissions) {
    ...
    public static EOSKeystore create(Metadata metadata, String password, String accountName,
            List<String> prvKeys, List<PermissionObject> permissions) {
    ...
    private static List<KeyPath> calcDefaultKeys(DeterministicKeyChain rootKeyChain, String path) {
        ...
        for (String subpath : subpaths) {
            byte[] prvKeyBytes = rootKeyChain.getKeyByPath(BIP44Util.generatePath(subpath),
                    true).getPrivKeyBytes();
    ...
    private void derivedKeyPath(List<KeyPath> keyPaths, List<PermissionObject> permissions,
            String password) {
        ...
        for (KeyPath keyPath : keyPaths) {
            ...
            String derivedMode = Strings.isNullOrEmpty(keyPath.getPath()) ? KeyPathPrivate.HD_SHA256
                    : KeyPathPrivate.PATH_DIRECTLY;
            KeyPathPrivate keyPathPrivate = new KeyPathPrivate(encPair, publicKey,
                    keyPath.getPath(), derivedMode);
            mKeyPathPrivates.add(keyPathPrivate);
        }
        if (permissions != null) {
            for (PermissionObject perm : permissions) {
                if (PERM_OWNER.equalsIgnoreCase(perm.permission) || PERM_ACTIVE.equalsIgnoreCase(
                        perm.permission)) {
    ...
    public static class KeyPathPrivate {
        ...
        public KeyPathPrivate(EncPair encPrivate, String publicKey, String path,
                String derivedMode) {
