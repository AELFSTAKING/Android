...
//  }
     ...
     */
    public static Wallet importWalletFromPrivateKeys(Metadata metadata, String accountName,
            List<String> prvKeys, List<EOSKeystore.PermissionObject> permissions, String password,
            boolean overwrite) {
     ...
     */
    @Deprecated
    public static Wallet importWalletFromPrivateKey(Metadata metadata, String accountName,
            String prvKeyHex, String password, boolean overwrite) {
     ...
     */
    public static Wallet importWalletFromMnemonic(Metadata metadata, @Nullable String accountName,
            String mnemonic, String path, @Nullable List<EOSKeystore.PermissionObject> permissions,
            String password, boolean overwrite) {
        ...
        switch (metadata.getChainType()) {
            ...
            case ChainType.BITCOIN:
                ...
                break;
            case ChainType.EOS:
                keystore = EOSKeystore.create(metadata, password, accountName, mnemonicCodes, path,
                        permissions);
        }
    ...
    public static Wallet findWalletByPrivateKey(String chainType, String network, String privateKey,
        ...
        String address = AddressCreatorManager.getInstance(chainType, net.isMainnet(),
                segWit).fromPrivateKey(privateKey);
    ...
    public static Wallet findWalletByMnemonic(String chainType, String network, String mnemonic,
        ...
        Network net = new Network(network);
        String address = AddressCreatorManager.getInstance(chainType, net.isMainnet(),
                segWit).fromPrivateKey(key.getPrivateKeyAsHex());
        return findWalletByAddress(chainType, address);
    }
    ...
    public static Wallet switchBTCWalletMode(String id, String password, String model) {
        ...
        if (wallet.hasMnemonic()) {
            ...
            keystore = new HDMnemonicKeystore(metadata, password, mnemonicCodes, path,
                    wallet.getId());
        } else {
    ...
    // Tick.Du adds the code.
    private static List<IMTKeystore> findKeystore(String type) {
        List<IMTKeystore> ret = new ArrayList<>();
        for (IMTKeystore keystore : keystoreMap.values()) {
            Log.i("Dupeng", " keystore.getAddress() : " + keystore.getAddress());
            if (Strings.isNullOrEmpty(keystore.getAddress())) {
                continue;
            }
            Log.i("Dupeng", " getChainType : " + keystore.getMetadata().getChainType());
            if (keystore.getMetadata().getChainType().equals(type)) {
                ret.add(keystore);
            }
        }
        return ret;
    ...
    // Tick.Du adds the code.
    public static List<Wallet> findWallet(String type) {
        List<Wallet> ret = new ArrayList<>();
        List<IMTKeystore> keystoreList = findKeystore(type);
        if (keystoreList != null && keystoreList.size() > 0) {
            for (IMTKeystore keystore : keystoreList) {
                ret.add(new Wallet(keystore));
            }
        }
        return ret;
    }
    ...
    private static IMTKeystore findKeystoreByAddress(String type, String address) {
        ...
        for (IMTKeystore keystore : keystoreMap.values()) {
            ...
            if (keystore.getMetadata().getChainType().equals(type) && keystore.getAddress().equals(
                    address)) {
                return keystore;
            }
        }
    ...
    private static V3Keystore validateKeystore(String keystoreContent, String password) {
        ...
        if (Strings.isNullOrEmpty(address) || !address.equalsIgnoreCase(
                importedKeystore.getAddress())) {
            ...
            if (!file.getName().startsWith("identity")) {
                try {
                    ...
                    if (version == 3) {
                        ...
                        } else if (jsonObject.has("imTokenMeta") && ChainType.EOS.equals(
                                jsonObject.getJSONObject("imTokenMeta").getString("chainType"))) {
