 ...
 */
public class HpbAddressCreator implements AddressCreator {
    ...
    public String fromPublicKey(BigInteger publicKey) {
        byte[] pubKeyBytes = NumericUtil.bigIntegerToBytesWithZeroPadded(publicKey,
                PUBLIC_KEY_SIZE);
    ...
    private String publicKeyToAddress(byte[] pubKeyBytes) {
        ...
        byte[] addrBytes = Arrays.copyOfRange(hashedBytes, hashedBytes.length - 20,
                hashedBytes.length);
        // Tick.Du modify this code.
        //return NumericUtil.bytesToHex(addrBytes);
        return NumericUtil.prependHexPrefix(NumericUtil.bytesToHex(addrBytes));
    }
