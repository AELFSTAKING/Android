...
public final class HDMnemonicKeystore extends IMTKeystore implements EncMnemonicKeystore {
    ...
    public HDMnemonicKeystore(Metadata metadata, String password, List<String> mnemonics,
        ...
        DeterministicKey mainAddressKey = keyChain.getKeyByPath(
                BIP44Util.generatePath(path + "/0/0"), true);
        if (Metadata.P2WPKH.equals(metadata.getSegWit())) {
            this.address = new SegWitBitcoinAddressCreator(networkParameters).fromPrivateKey(
                    mainAddressKey.getPrivateKeyAsHex());
        } else {
        ...
        this.crypto = Crypto.createPBKDF2CryptoWithKDFCached(password,
                xprv.getBytes(Charset.forName("UTF-8")));
    ...
    public static HDMnemonicKeystore create(Metadata metadata, String password,
            List<String> mnemonics, String path) {
    ...
    public String newReceiveAddress(int nextIdx) {
        NetworkParameters networkParameters =
                this.metadata.isMainNet() ? MainNetParams.get() : TestNet3Params.get();
        ...
        DeterministicKey indexKey = HDKeyDerivation.deriveChildKey(changeKey,
                new ChildNumber(nextIdx));
        if (Metadata.P2WPKH.equals(metadata.getSegWit())) {
            return new SegWitBitcoinAddressCreator(networkParameters).fromPrivateKey(
                    indexKey).toBase58();
        } else {
