...
import io.tick.base.net.response.BaseResponseBean;
...
public class JsonConvert<T> implements Converter<T> {
    ...
    private T parseParameterizedType(okhttp3.Response response, ParameterizedType type)
            throws Exception {
        if (type == null) return null;
        ResponseBody body = response.body();
        if (body == null) return null;
        ...
        if (rawType != ResponseBean.class) {
            ...
            if (typeArgument == Void.class) {
                ...
                BaseResponseBean baseResponseBean = Convert.fromJson(jsonReader,
                ...
                ResponseBean MResponseBean = Convert.fromJson(jsonReader, type);
                response.close();
                return (T) MResponseBean;
            }
        }
    }
}
