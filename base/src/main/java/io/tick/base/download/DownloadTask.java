...
public class DownloadTask extends Handler {
    ...
    private final int THREAD_COUNT = 6;//线程数
        ...
        switch (msg.what) {
            case MSG_PROGRESS://进度
                ...
                for (int i = 0, length = mProgress.length; i < length; i++) {
                    log.append("P[").append(i).append("]").append("=").append(
                            mProgress[i]).append(",");
                    progress += mProgress[i];
                }
                Log.i("Tick", "fileLength=" + mFileLength + ",progress=" + progress + ","
                        + log.toString());
            ...
            case MSG_FINISH://完成
                ...
                mTmpFile.renameTo(
                        new File(mPoint.getFilePath(), mPoint.getFileName()));//下载完毕后，重命名目标文件名
                resetStatus();
    ...
    public synchronized void start() {
        try {
            Log.i(TAG, "start: " + isDownloading + "\t" + mPoint.getUrl());
            ...
            mHttpUtil.getContentLength(mPoint.getUrl(), new okhttp3.Callback() {
                ...
                public void onResponse(Call call, Response response) throws IOException {
                    Log.i(TAG,
                            "start: " + response.code() + "\t isDownloading:" + isDownloading + "\t"
                                    + mPoint.getUrl());
    ...
    private void download(final long startIndex, final long endIndex, final int threadId)
        ...
        mHttpUtil.downloadFileByRange(mPoint.getUrl(), finalStartIndex, endIndex,
                new okhttp3.Callback() {
                    ...
                    public void onResponse(Call call, Response response) throws IOException {
                        Log.e(TAG,
                                "download: " + response.code() + "\t isDownloading:" + isDownloading
                                        + "\t" + mPoint.getUrl());
                        ...
                        RandomAccessFile tmpAccessFile = new RandomAccessFile(mTmpFile,
                                "rw");// 获取前面已创建的文件.
