...
public final class ViewfinderView extends View {
    ...
    private void initInnerRect(Context context, AttributeSet attrs) {
        ...
        // 扫描框边角宽度
        innercornerwidth = (int) ta.getDimension(R.styleable.ViewfinderView_inner_corner_width, 4);
        System.out.println("width= " + innercornerwidth);
    ...
    public void onDraw(Canvas canvas) {
        ...
        //画外周灰色部分
        drawBg(canvas, frame, width, height);

        if (resultBitmap != null) {
            // Draw the opaque result bitmap over the scanning rectangle
            paint.setAlpha(OPAQUE);
            Point screenResolution = cameraManager.getScreenResolution();
            canvas.drawBitmap(resultBitmap, screenResolution.x, screenResolution.y, paint);
        } else {
            //画边框及四角
            ...
            if (currentPossible.isEmpty()) {
                ...
                if (isCircle) {
                    for (ResultPoint point : currentPossible) {
                        canvas.drawCircle((float) (frame.left + Math.random() * frame.width()), (float) (frame.top + Math.random() * frame.height()), 6.0f, paint);
                    }
                }
            }
            if (currentLast != null) {
                ...
                if (isCircle) {
                    for (ResultPoint point : currentLast) {
                        canvas.drawCircle((float) (frame.left + Math.random() * frame.width()), (float) (frame.top + Math.random() * frame.height()), 3.0f, paint);
                    }
                }
            }
    ...
    private void drawBg(Canvas canvas, Rect frame, int width, int height) {
...
//        int halfCorWidth = corWidth / 2;
//        paint.setColor(Color.WHITE);
//        paint.setStrokeWidth(corWidth);
//        paint.setStyle(Paint.Style.STROKE);

//        canvas.drawRect(frame.left + halfCorWidth, frame.top + halfCorWidth, frame.right - halfCorWidth, frame.bottom - halfCorWidth, paint);
